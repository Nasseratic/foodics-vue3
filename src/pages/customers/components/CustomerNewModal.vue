...
import { FieldTypes, NumberField, TextField } from "@asigloo/vue-dynamic-forms";
import { useCreate, useUpdate } from "../composables/useCustomers";
import { useRoute } from "vue-router";
import { useCustomersForm } from "../composables/useCustomersForm";
...
export default defineComponent({
  ...
  setup({ close }) {
    ...
    const form = useCustomersForm();

    async function formSubmitted(values: any) {
      if (route.params.id) await updateUser(values);
